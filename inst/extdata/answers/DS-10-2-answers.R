...
# ---
practice.begin("DS-10-2", learner="[your name]")
...
#    to add a number to the word
#    "Employee" (employees)
employees <- paste("Employee", 1:100)

# Note: A vector, named `salaries_2017`, has been created with with this command:
#       salaries_2017 <- round(runif(100, 40000, 50000),2)
#    The function `runif()` is used to create 100 random numbers between
#    $40,000 and $50,000. The function `round()` is used to round the numbers
#    to two decimal places. Try running this command to see that it works:
#       round(runif(100, 40000, 50000),2)



# Note: A vector, named `salary_adjustments`, has also been created, with this command:
#       salary_adjustments <- round(runif(100, -5000, 10000),2)
#    These salary adjustments range from negative $5,000 to positive $10,000. Try
#    running this command to see that it works:
...
#       salary_adjustments
#
#    Remember to set `stringsAsFactors=FALSE`! (salaries)
salaries <- data.frame(employees, salaries_2017, salary_adjustments, stringsAsFactors = FALSE)

# Note: Add a column, named `salaries_2018`, to the `salaries` dataframe that
...
# Note: Add a column to the `salaries` data frame that has a value of `TRUE` if the
...
#    variable`all_done` (all_done)
all_done <- salaries

# Note: Retrieve values from your data frame to answer the following questions Note
#    that you should get the value as specific as possible (e.g., a single cell
#    rather than the whole row!)



# d: What was the 2018 salary of Employee 57. (salary_57)
salary_57 <- salaries[salaries$employees == "Employee 57", "salaries_2018"]

# e: How many employees got a raise? Assign the answer to the variable `num_got_raise`. (num_got_raise)
num_got_raise <- nrow(salaries[salaries$got_raise == TRUE, ])

# f: What was the dollar value of the highest raise? (highest_raise)
highest_raise <- max(salaries$salary_adjustments)

# g: What was the `name` of the employee who received the highest raise? (got_biggest_raise)
got_biggest_raise <- salaries[salaries$salary_adjustments == highest_raise, "employees"]

# h: What was the largest decrease in salaries between the two years? (biggest_paycut)
biggest_paycut <- min(salaries$salary_adjustments)

# i: What was the name of the employee who received largest decrease in salary? (got_biggest_paycut)
got_biggest_paycut <- salaries[salaries$salary_adjustments == biggest_paycut, "employees"]

# j: What was the average salary change? (avg_increase)
avg_increase <- mean(salaries$salary_adjustments)

# k: For people who did not get a raise, how much money did they lose on average? (avg_loss)
avg_loss <- mean(salaries$salary_adjustments[salaries$got_raise == FALSE])

# Note: Consider: do the above averages match what you expected them to be based on
#    how you generated the salaries?



# Note: Some final things:
#        (1) What is your "working directory?" You can find out with this
#            command: `wd()`
#        (2) Write your salary dataframe out to a file in your working
#            directory. Recall the `write()` command:
#                write.csv(salaries,"salaries.csv")
#        (3) Read your file back into R with the `read()` command. For example,
#                input <- read.csv("salaries.csv")
#        (4) Use `View(input)` to view the dataframe in RStudio.
#    Note: "csv" refers to "comma separated values." Open the "salaries.csv"
#          file to confirm.
